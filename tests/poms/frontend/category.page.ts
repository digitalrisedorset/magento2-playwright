...
class CategoryPage {
  ...
  async goToCategoryPage(){
    ...
    const filters = this.page.locator('.filter-options-item');

    const firstFilter = filters.first();
    await expect(
      firstFilter.locator('.filter-options-content')
    ).toHaveAttribute('aria-hidden', 'true');
    await firstFilter.locator('.filter-options-title').click();
    await expect(
      firstFilter.locator('.filter-options-content')
    ).toBeVisible();
   ...
   */
  async filterOnSize() {
    const sizeFilterButton = this.page.getByRole('tab', {name: UIReference.categoryPage.sizeFilterButtonLabel});
    //const sizeFilterButton = this.page.locator('[data-role="collapsible"]', { has: this.page.locator('[data-role="title"]', { hasText: 'Size' }) });
    const sizeFilter = sizeFilterButton.locator('xpath=ancestor::*[@data-role="collapsible"]');
    const sizeFilterContent = sizeFilter.locator('[data-role="content"]');
